...
const getClients = (params) => {
  return api.get(`/api/client/${params}`);
};
const getClientsFormsPage = (params) => {
  return api.get(`/api/client/get-forms-clients/${params}`);
};
const getClientsFormsDetailPage = (params) => {
  return api.get(`/api/client/get-forms-client-detail/${params}`);
};

const getClient = (id, params = "") => {
  const lang = localStorage.getItem("DEFAULT_LANGUAGE");
  return api.get(`/api/client/${id}/?lang=${lang}&${params}`);
};
...
const processPdfTemplate = (clientId, templateData) => {
  return api.post(
    `/api/client/${clientId}/process-pdf-template/`,
    templateData
  );
};

const fetchClientOpenUrls = (urlSearch) => {
  return api.get(`/api/client/client-search?target_url=${urlSearch}`);
};

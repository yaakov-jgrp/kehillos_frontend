                        ...
                        <div className="relative w-[90%] my-6 mx-auto max-w-[90%]">
                            <div className="min-w-300px md:min-w-[400px] p-4 max-h-[90svh] overflow-y-auto border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                <div className="flex items-center justify-between p-5 border-b border-solid border-gray-300 rounded-t ">
                                    ...
                                    <div className="flex">
                                        <label className={`w-fit rounded-full flex items-center py-1 px-3 mr-1 mt-2 text-[12px] font-medium bg-brand-500 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 text-white dark:hover:bg-brand-300 dark:active:bg-brand-200`} onClick={exportClientsSampleHandler}>
                                            <FaDownload className={`rounded-full text-white  ${lang === "he" ? "ml-1" : "mr-1"} w-3 h-3 hover:cursor-pointer`} />
                                ...
                                >
                                    {
                                        formFields.length > 0 && formFields.map((field, index) => {
                                            return (
                                                <ImporterField key={index} name={field?.field_slug} label={field?.field_name} optional={!field?.required} />
                                            )
                                        })
                                    }
                                </Importer>

...
export default function SearchFilterModal({
  ...
  const handleSwitchChange = (switchName) => {
    setActiveSwitch((prev) => {
      if (prev.includes(switchName)) {
        if (prev.length === 1) {
          return prev; // Prevent deselecting the last remaining option
        }
        return prev.filter((item) => item !== switchName);
      } else {
        return [...prev, switchName];
      }
    });
  };
          ...
          <div className="min-w-300px md:min-w-[400px] overflow-y-auto border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            ...
            <div className="relative p-6 flex-auto max-h-[calc(90vh-170px)] overflow-y-auto">
              {/* <div className="flex items-center justify-between mb-4">
                <span>Name</span>
                <ToggleSwitch
                  selected={activeSwitch.includes("name")}
              ...
              <div className="flex items-center justify-between mb-4">
                <span>Email</span>
                <ToggleSwitch
                  selected={activeSwitch.includes("email")}
              ...
              <div className="flex items-center justify-between mb-4">
                <span>ID</span>
                <ToggleSwitch
                  selected={activeSwitch.includes("id")}
                  clickHandler={() => handleSwitchChange("id")}
                />
              </div> */}

              {allClientsFields.map((field, index) => (
                <div
                  key={index}
                ...
                >
                  <span>{field}</span>
                  <ToggleSwitch
                    selected={activeSwitch.includes(field)}
                    clickHandler={() => handleSwitchChange(field)}
                  />
                </div>
              ))}
            </div>

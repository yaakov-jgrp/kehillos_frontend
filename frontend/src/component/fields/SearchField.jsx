    ...
    <div className={`${extra} ${(i18n.dir() === 'rtl') ? 'text-right' : 'text-left'}`}>
      ...
      <div className="flex justify-center items-end relative">
        <input
          ...
          className={`mt-2 h-7 w-full px-6 text-[14px] bg-transparent  outline-none bg-lightPrimary ${!noUnderline && 'border-b-[1px] border-[#2B3674]'} ${borderRadius && "rounded-full"}`}
        />
        <span className={`absolute ${(i18n.dir() === 'rtl') ? variant === "auth" ? 'right-1' : 'right-[10px]' : variant === "auth" ? 'left-1' : 'left-[10px]'} top-3/5 transform -translate-y-1/2`}>

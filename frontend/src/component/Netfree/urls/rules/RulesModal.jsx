...
function RulesModal({
  ...
  const { mutate: createRuleHandler, isPending: isCreatingRule } = useMutation({
    mutationKey: ["addRule"],
    mutationFn: (data) => rulesServices.createRule(data),
  });

  const { mutate: editRuleHandler, isPending: isEditingRule } = useMutation({
    mutationKey: ["editRule"],
    mutationFn: (data) => rulesServices.editRule(data, editRule.id),
  });
  ...
  useEffect(() => {
    if (editRule) {
      ...
      setValue("action", editRule.action);
    }
  }, [editRule]);
